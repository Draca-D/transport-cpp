...
RETURN_CODE NetworkDevice::sendTo(const HostAddr &dest, const IODATA &message,
...
RETURN_CODE NetworkDevice::sendTo(const HostAddr &dest,
...
RETURN_CODE NetworkDevice::syncSendTo(const HostAddr &dest,
...
RETURN_CODE NetworkDevice::getLocalAddress(HostAddr &addr) noexcept {
  if (!deviceIsReady()) {
    setError(ERROR_CODE::INVALID_LOGIC,
             "Device is not ready, unable to get local address");
    return RETURN::NOK;
  }
  ...
  if (local_addr.sa_family == AF_INET) {
    ...
    if (inet_ntop(AF_INET, &ipv4_addr->sin_addr, ip_buffer,
    ...
    if (inet_ntop(AF_INET6, &ipv6_addr->sin6_addr, ip_buffer,
                  sizeof(ip_buffer)) == nullptr) {
      setError(errno, "Unable to convert binary address to string");
      return RETURN::NOK;
    }
...
Device::ERROR NetworkDevice::receiveMessage(NetworkMessage &message) const {
  ...
  auto nbytes =
      recvfrom(handle, buffer, RECV_BUFFER_LEN, 0,
  ...
  if (nbytes == -1) {
    ...
    if (peer_addr.sa_family == AF_INET) {
      ...
      if (inet_ntop(AF_INET, &ipv4_addr->sin_addr, ip, IP_NAME_BUF_LEN) ==
          nullptr) {
        err.code = errno;
      ...
      if (inet_ntop(AF_INET6, &ipv6_addr->sin6_addr, ip, IP_NAME_BUF_LEN) ==
          nullptr) {
        err.code = errno;
  ...
  while (nbytes > 0) {
    ...
    nbytes = recvfrom(handle, buffer, RECV_BUFFER_LEN, 0, &peer_addr,
                      &peer_addr_len);
  }
...
RETURN_CODE
NetworkDevice::createAndConnectSocket(const HostAddr &host,
                                      const IPVersion &ip_hint,
  ...
  if (const auto status = info.loadHints(hints, host); status != 0) {
    setError(ERROR_CODE::GENERAL_ERROR,
             std::string("unable to get address information: ") +
                 gai_strerror(status));
  ...
  for (; next_info != nullptr; next_info = info.next()) {
    auto sock = socket(next_info->ai_family, next_info->ai_socktype,
    ...
    if (const auto res =
            connect(sock, next_info->ai_addr, next_info->ai_addrlen);
        res != 0) {
      close(sock);
      continue;
    }
...
RETURN_CODE NetworkDevice::createAndBindSocket(const HostAddr &host,
                                               const IPVersion &ip_hint,
                                               const SOCK_STYLE &sock_style) {
  AddrInfo info;
...
void NetworkDevice::readyRead() {
  ...
  if (!std::holds_alternative<ERROR_CODE>(read_resp.code) ||
    ...
    logError("NetworkDevice/readyRead",
...
void NetworkDevice::readyWrite() {
  ...
  if (ret == RETURN::NOK) {
    ...
    if (std::holds_alternative<int>(last_error.code)) {
      logError("NetworkDevice/readyWrite",
               "[Sys] Unable to send to an address. "
               "Error code description: " +
                   std::string(strerror(std::get<int>(last_error.code))));
    } else {
      logError("NetworkDevice/readyWrite",
               "[Internal error] Unable to send to. Desc: " +
...
RETURN_CODE NetworkDevice::performSendTo(const HostAddr &dest,
                                         const IODATA_CHOICE &data,
                                         const IPVersion &ip_hint) {
  if (!getDeviceHandle()) {
    setError(ERROR_CODE::INVALID_LOGIC,
             "Cannot send without first initialising a socket");
    return RETURN::NOK;
  }
  ...
  auto nWrote = sendto(sock, data_ptr->data(), data_ptr->size(), 0,
                       next_info->ai_addr, next_info->ai_addrlen);
...
IFACE_LIST getAllInterfaces() {
  ...
  for (ifa = ifAddrStruct; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if (ifa->ifa_addr->sa_family == AF_INET) {
      ...
      tmpMaskPtr =
          &(reinterpret_cast<sockaddr_in *>(ifa->ifa_netmask))->sin_addr;
      ...
      interfaces.push_back(
          {ifa->ifa_name, addressBuffer, netmaskBuffer, IPVersion::IPv4});
      ...
      interfaces.push_back(
          {ifa->ifa_name, addressBuffer, netmaskBuffer, IPVersion::IPv6});
    }
  }
...
ADDR getLocalBroadcasterAddr(const std::string &if_name) {
  ...
  for (ifa = ifAddrStruct; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    if (ifa->ifa_addr->sa_family == AF_INET) {
      ...
      const auto ipv4_addr =
          reinterpret_cast<const in_addr *>(tmpAddrPtr)->s_addr;
      const auto ipv4_mask =
          reinterpret_cast<const in_addr *>(tmpMaskPtr)->s_addr;

      const auto broadcast_addr = ipv4_addr | ~ipv4_mask;

      char localBroadcastBuffer[INET_ADDRSTRLEN];

      inet_ntop(AF_INET, &broadcast_addr, localBroadcastBuffer,
                INET_ADDRSTRLEN);
  ...
  if (if_exists && addr.empty()) {
    throw std::runtime_error(
        "Provided interface name exists but does not have an ipv4 address. "
...
RETURN_CODE NetworkDevice::sockToReuse(const DEVICE_HANDLE &handle) {
  if (!handle) {
    setError(ERROR_CODE::INVALID_LOGIC,
             "reuse requested, but no device handle present");
    return RETURN::NOK;
  }
...
AddrInfo::AddrInfoReturn AddrInfo::loadHints(const addrinfo &hints,
                                             const HostAddr &addr) {
  if (mInfo) {
    freeaddrinfo(mInfo);
    mInfo = nullptr;
  ...
  int status = getaddrinfo(addr.ip.c_str(), std::to_string(addr.port).c_str(),
                           &hints, &mInfo);
