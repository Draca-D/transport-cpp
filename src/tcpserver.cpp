...
bool Acceptor::isBound() const noexcept
{
    return mIsBound;
}
...
{
    if(!getDeviceHandle()){
        logWarn("Acceptor/listen", "Listen requested, but no device handle present");
        return;
    }
...
void Acceptor::readyRead()
{
    ...
    case AF_INET:
        inet_ntop(AF_INET, &((reinterpret_cast<struct sockaddr_in *>(&their_addr))->sin_addr),
                  ip, static_cast<socklen_t>(maxlen));
        port = ntohs((reinterpret_cast<struct sockaddr_in *>(&their_addr))->sin_port);
        break;

    case AF_INET6:
        inet_ntop(AF_INET6, &((reinterpret_cast<struct sockaddr_in6 *>(&their_addr))->sin6_addr),
                  ip, static_cast<socklen_t>(maxlen));
        port = ntohs((reinterpret_cast<struct sockaddr_in6 *>(&their_addr))->sin6_port);
        break;
...
void Peer::readyRead()
{
    ...
    if(!std::holds_alternative<ERROR_CODE>(read_resp.code) ||
        ...
        logError("TCPPeer/readyRead", "Error reading descriptor. " + read_resp.description);
        return;
    }
...
void Peer::readyHangup()
{
    peerDisconnected();
}

void Peer::readyPeerDisconnect()
{
    peerDisconnected();
}
...
void Peer::notifyServerHandler(const NetworkMessage &request)
{
    ...
    if(ret == RETURN::NOK){
        ...
        if(std::holds_alternative<ERROR_CODE>(error.code)){
            ...
            err_desc = "SYS error (code: " + std::to_string(err_code) + " | sys description: " +
                    strerror(err_code) + "). Description: ";
        }
...
HostAddr Peer::getPeerAddr() const noexcept
{
    return mPeerAddr;
}
