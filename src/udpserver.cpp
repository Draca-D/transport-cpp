...
namespace Context::Devices::IO::Networking::UDP {
    ...
    HostAddr Peer::getPeerAddress() const noexcept {
        return mPeerAddr;
    }

    bool Peer::isValid() const noexcept {
        return mIsValid;
    }

    RETURN_CODE Peer::sendTo(const HostAddr &dest, const IODATA &message, const IPVersion &ip_hint) {
        if (mAsyncSendPlain) {
            return mAsyncSendPlain(dest, message, ip_hint);
        }

        setError(ERROR_CODE::DEVICE_NOT_READY, "This peer requires a valid server instance");
        return RETURN::NOK;
    }

    RETURN_CODE Peer::sendTo(const HostAddr &dest, const std::shared_ptr<IODATA> &message, const IPVersion &ip_hint) {
        if (mAsyncSendShared) {
    ...
    RETURN_CODE Peer::sendTo(const HostAddr &dest, std::unique_ptr<IODATA> message, const IPVersion &ip_hint) {
        if (mAsyncSendUnique) {
    ...
    RETURN_CODE Peer::syncSendTo(const HostAddr &dest, const IODATA_CHOICE &message, const IPVersion &ip_hint) {
        if (mSyncSend) {
    ...
    RETURN_CODE Server::asyncSend(const IODATA &data) {
        if(!mPeerConnected) {
            setError(ERROR_CODE::DEVICE_NOT_READY, "We need to first receive a message from a peer before we can send messages");
            return RETURN::NOK;
        }
    ...
    void Server::readyRead() {
        ...
        if (!std::holds_alternative<ERROR_CODE>(read_resp.code) ||
            ...
            logError("UDPServer/readyRead", "Error reading descriptor. " + read_resp.description);
            return;
        }

        notifyCallback(data);

        auto relevant_peer = std::find_if(mPeers.begin(), mPeers.end(), [&data](const Peer *pr) {
        ...
        if (relevant_peer == mPeers.end()) {
            ...
            new_peer->mAsyncSendPlain = [this](const HostAddr &dest, const IODATA &message,
                                               const IPVersion &ip_hint) -> RETURN_CODE {
                return sendTo(dest, message, ip_hint);
            };

            new_peer->mAsyncSendShared = [this](const HostAddr &dest, const std::shared_ptr<IODATA> &message,
                                                const IPVersion &ip_hint) -> RETURN_CODE {
                return sendTo(dest, message, ip_hint);
            };

            new_peer->mAsyncSendUnique = [this](const HostAddr &dest, std::unique_ptr<IODATA> message,
                                                const IPVersion &ip_hint) -> RETURN_CODE {
                return sendTo(dest, std::move(message), ip_hint);
            };

            new_peer->mSyncSend = [this](const HostAddr &dest, const IODATA_CHOICE &message,
                                         const IPVersion &ip_hint) -> RETURN_CODE {
