...
namespace Context::Devices::IO::Networking::UDP {
    ...
    RETURN_CODE Multicaster::publishToGroup(const HostAddr &group) {
        if (!mInitalised) {
            setError(ERROR_CODE::INVALID_LOGIC, "Multicaster has not been initialised yet");
            return RETURN::NOK;
        }
        ...
        if (mIpVersion == IPVersion::IPv4) {
            ...
            setError(ERROR_CODE::GENERAL_ERROR, "Multicaster was somehow initialised as 'Any'");
            return RETURN::NOK;
        }
    ...
    RETURN_CODE Multicaster::setInterface(const std::string &iface_name) {
        ...
        const auto ipv4_iface = std::find_if(
            interfaces.begin(), interfaces.end(), [&iface_name, this](const IFACE &iface) {
                return (iface_name == iface.if_name) && (iface.ip_version == IPVersion::IPv4);
            });

        const auto ipv6_iface = std::find_if(
            interfaces.begin(), interfaces.end(), [&iface_name, this](const IFACE &iface) {
        ...
        if (mIpVersion == IPVersion::IPv4 && ipv4_iface == interfaces.end() && ipv6_iface != interfaces.end()) {
            setError(ERROR_CODE::INVALID_LOGIC, "Multicaster was initialised as ipv4 but provided interface "
                     "only supports ipv6");
            return RETURN::NOK;
        }

        if (mIpVersion == IPVersion::IPv6 && ipv6_iface == interfaces.end() && ipv4_iface != interfaces.end()) {
            setError(ERROR_CODE::INVALID_LOGIC, "Multicaster was initialised as ipv6 but provided interface "
                     "only supports ipv4");
            return RETURN::NOK;
        }
