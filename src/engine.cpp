...
RETURN_CODE
Context::Engine::registerDevice(std::shared_ptr<Device> device) noexcept {
  if (!device) {
    setError(
        ERROR_CODE::INVALID_ARGUMENT,
        "When registering a new Device, an invalid sharedptr was provided");
    return RETURN::NOK;
  }
...
RETURN_CODE Engine::deRegisterDevice(std::shared_ptr<Device> device) noexcept {
  if (!device) {
    setError(ERROR_CODE::INVALID_ARGUMENT,
             "When deregistering a Device, an invalid sharedptr was provided");
    return RETURN::NOK;
  }
...
void Engine::setLogger(const LOGGER &logger) noexcept { mLogger = logger; }

Engine::ERROR Engine::getLastError() const noexcept { return mLastError; }

void Engine::awaitOnce(
    const std::optional<std::chrono::milliseconds> &optional_duration) {
  int timeout = -1;
  ...
  if (optional_duration) {
    ...
    if (duration.count() > std::numeric_limits<decltype(timeout)>::max()) {
      logWarn(
          "Engine/awaitOnce",
          "Provided timeout exceeds the system max duration of " +
              std::to_string(std::numeric_limits<decltype(timeout)>::max()) +
              " milliseconds. Clamping to max");
...
RETURN_CODE Engine::registerNewHandle(DEVICE_HANDLE old_handle,
...
RETURN_CODE Engine::deRegisterHandle(DEVICE_HANDLE handle) {
  ...
  if (handle_pos == mPollDevices.end()) {
    setError(ERROR_CODE::DEVICE_DOES_NOT_EXIST,
             "When deregistering the handle, "
...
Engine::POLL_LIST::iterator Engine::findHandleInList(DEVICE_HANDLE handle) {
  if (!handle.has_value()) {
  ...
  return std::find_if(mPollDevices.begin(), mPollDevices.end(),
                      [&handle](const decltype(mPollDevices)::value_type &fd) {
                        return fd.fd == handle.value();
                      });
}
...
void Engine::logDebug(const std::string &calling_class,
...
void Engine::logInfo(const std::string &calling_class,
                     const std::string &message) const {
  if (mLogger) {
...
void Engine::logWarn(const std::string &calling_class,
                     const std::string &message) const {
  if (mLogger) {
...
void Engine::logError(const std::string &calling_class,
                      const std::string &message) const {
  if (mLogger) {
...
void Engine::logFatal(const std::string &calling_class,
                      const std::string &message) const {
  if (mLogger) {
